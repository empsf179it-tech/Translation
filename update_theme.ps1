...
$cssContent = $cssContent -replace '#fff', 'var(--text-main)'
$cssContent = $cssContent -replace '#020610', 'var(--footer-bg)'
$cssContent = $cssContent -replace 'rgba\(255, 255, 255', 'rgba(var(--rgb-white)'
$cssContent = $cssContent -replace 'rgba\(4, 11, 22', 'rgba(var(--rgb-dark-bg)'
$cssContent = $cssContent -replace 'rgba\(0, 0, 0', 'rgba(var(--rgb-black)'
...
$cssContent = $cssContent -replace ':root \{', ":root {`n$rootVars"
...
if ($jsContent -notmatch '// Theme Toggle Logic') {
    $jsContent = $jsContent -replace "document.addEventListener\('DOMContentLoaded', \(\) => \{", "document.addEventListener('DOMContentLoaded', () => {`n$jsThemeLogic"
...
foreach ($file in $htmlFiles) {
    $htmlContent = Get-Content -Raw -Path $file.FullName
    $htmlContent = $htmlContent -replace '</head>', $themeScript
    $htmlContent = $htmlContent -replace '<div class="nav-cta">', "<div class=`"nav-cta`" style=`"display: flex; align-items: center;`">`n$themeBtnHtml"
    Set-Content -Path $file.FullName -Value $htmlContent
}

Write-Output "PowerShell theme update complete."
